...
public class Sort{
	...
	public void quicksortl(int start, int end){
		if(start >= end) return;
		// end is the pivot here
		int pivot = end;
		int left = start;
		int right = pivot-1;
		while(left<right){
			// you must be careful about this. the index may be out of bound
			// As left >= right is possible in this loop, so don't use left<right
			// as the constraint. Use left < pivot and right >= start.
		...
		quicksortl(start, pivot-1);
		quicksortl(pivot+1, end);
	}
	...
	*/
	...
	public static void main(String[] args){
		...
		sort.quicksortl(0, arr.length-1);
		//sort.bubblesort();
		//sort.selectionsort();
		//sort.insertionsort();
		sort.print();
